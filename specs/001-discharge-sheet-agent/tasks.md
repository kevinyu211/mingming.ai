...
- [ ] T042 [P] Deploy to Vercel (project link, env vars `ANTHROPIC_API_KEY`, provider keys, `MODEL_*`, `TTS_PROVIDER`, `STT_PROVIDER`), verify from a phone on mobile data, generate a QR code to `docs/qr.png`, and document the laptop fallback (`next build && next start`, hotspot, LAN QR) in `README.md`

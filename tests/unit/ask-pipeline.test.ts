...
describe("grounding is enforced server-side", () => {
  ...
  it("falls back to the template when the model says grounded:false", async () => {
    answer.mockResolvedValue({
      result: { grounded: false, citedCardId: "medicine-0", answer: CLEAN_ANSWER },
      usage: USAGE,
    });
  ...
  it("falls back to the template when the cited card id does not exist", async () => {
    answer.mockResolvedValue({
      result: { grounded: true, citedCardId: "medicine-99", answer: CLEAN_ANSWER },
      usage: USAGE,
    });
  ...
  it("falls back to the template when the citation is null", async () => {
    answer.mockResolvedValue({
      result: { grounded: true, citedCardId: null, answer: CLEAN_ANSWER },
      usage: USAGE,
    });
  ...
  it("falls back to the template when the answer is null", async () => {
    answer.mockResolvedValue({
      result: { grounded: true, citedCardId: "medicine-0", answer: null },
      usage: USAGE,
    });
...
describe("the banned-term filter runs on the answer", () => {
  it("regenerates once with the matched terms, then falls back to the template", async () => {
    answer.mockResolvedValue({
      result: { grounded: true, citedCardId: "medicine-0", answer: DIRTY_ANSWER },
      usage: USAGE,
    });
